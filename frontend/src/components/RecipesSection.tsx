    ...
    <section className="grid gap-6 lg:grid-cols-[1.1fr_1fr]">
      <article className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 p-6">
        <h2 className="text-xl font-semibold">
          {editingRecipeId !== null ? t("recipes.editTitle") : t("recipes.addTitle")}
        </h2>
        <form className="mt-4 grid gap-3" onSubmit={handleSubmit}>
          <input
            ...
            placeholder={t("recipes.form.name")}
          />
          <div className="grid grid-cols-2 gap-3">
            <input
              ...
              className={inputClass}
              placeholder={t("recipes.form.cuisine")}
            />
            <select
          ...
          <textarea
            ...
            className={inputClass}
            placeholder={t("recipes.form.instructions")}
            rows={3}
          />
          <button
            type="submit"
          ...
          >
            {editingRecipeId !== null
              ? updateRecipeMutation.isPending ? t("common.saving") : t("common.saveChanges")
              : addRecipeMutation.isPending ? t("common.adding") : t("recipes.form.add")}
          </button>
          {editingRecipeId !== null && (
            <button
              type="button"
          ...
          {addRecipeMutation.isError && (
            <p className="text-sm text-rose-600 dark:text-rose-300">{t("recipes.error.add")}</p>
          )}
          {updateRecipeMutation.isError && (
            <p className="text-sm text-rose-600 dark:text-rose-300">{t("recipes.error.update")}</p>
          )}
        </form>
      </article>

      <article className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 p-6">
        <h2 className="text-xl font-semibold">{t("recipes.library")}</h2>
        <div className="mt-4 space-y-3">
          {filteredRecipes.length === 0 ? (
            <p className="rounded-lg bg-slate-50 dark:bg-slate-800/70 p-4 text-slate-600 dark:text-slate-300">
              {searchQuery ? t("recipes.noMatch", { query: searchQuery }) : t("recipes.empty")}
            </p>
          ) : (
        ...
        {deleteRecipeMutation.isError && (
          <p className="mt-3 text-sm text-rose-600 dark:text-rose-300">{t("recipes.error.delete")}</p>
        )}
      </article>
    </section>
  );
